...
void main() {
  ...
  group('IRCTCParser SMS Parser', () {
    late IRCTCTrainParser irctcParser;

    setUp(() {
      irctcParser = IRCTCTrainParser();
    });
    ...
    test('should parse waitlisted IRCTC ticket to Ticket model correctly', () {
      ...
      // Status Tag
      final statusTag = ticket.tags?.firstWhere((t) => t.icon == 'info');
      expect(statusTag?.value, contains('WL 31')); // Status extraction now supports the number
    ...
    test('should parse old-format IRCTC SMS into Ticket model correctly', () {
      ...
      // The name extraction in this case should now be empty due to lack of a clear name pattern
      expect(
        ticket.extras?.firstWhere((e) => e.title == 'Passenger').value,
        equals(''),
      );
