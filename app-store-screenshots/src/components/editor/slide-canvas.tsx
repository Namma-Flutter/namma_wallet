...
function getDefaultRects(
  ...
  switch (layout) {
    ...
    case "device-bottom": {
      // For iPad the canvas is very tall; give the caption more breathing room
      const capTopFrac = device === "ipad" ? 0.05 : 0.08;
      const capHFrac   = device === "ipad" ? 0.22 : 0.28;
      return {
        caption: { x: cW * 0.08, y: cH * capTopFrac, width: capW, height: cH * capHFrac, align: "center" },
        device: {
          x: (cW - deviceW) / 2,
...
function getSlideGeometry(slide: Slide, device: Device, orientation: Orientation) {
  ...
  const defaults = getDefaultRects(slide.layout, cW, cH, frameAspect, fwFrac, fwSmallFrac, device);
